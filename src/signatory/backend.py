...
def signature(path: torch.Tensor, depth: int, basepoint: bool = False, stream: bool = False,
    ...
    The input :attr:`path` is expected to be a three-dimensional tensor, with dimensions :math:`(N, C, L)`, in the same
    way as, for example, torch.nn.Conv1D. That is, :math:`N` is the batch size, :math:`C` denotes the number of channels,
    and :math:`L` is the length of the input sequence. Each batch element is interpreted as a stream of data
    ...
    tensors of shape :math:`(C)`, :math:`(C, C)`, ... :math:`(C, \ldots, C)`, where the final tensor has :attr:`depth`
    many dimensions. If :attr:`flatten` is True then these are then flattened down and concatenated to give a single
    tensor of shape :math:`C + C^2 + \cdots + C^d`, where :math:`d` is the value of :attr:`depth`. (The value
    :math:`C + C^2 + \cdots + C^d` may be computed via the `signatory.signature_channels` function.)
    ...
    Thus for example if :attr:`stream`=``False``, :attr:`flatten`=``True`` then the returned tensor will have shape
    :math:`(N, C + C^2 + \cdots + C^d)`. If :attr:`basepoint=True`, :attr:`stream`=``True``, :attr:`flatten`=``True``,
    then the returned tensor will have shape :math:`(N, C + C^2 + \cdots + C^d, L)`, as the stream dimension is now
    preserved. See 'Returns', below, for a more precise description of exactly what is returned.

    Arguments:
        path (torch.Tensor): The input path to apply the signature transform to.
        depth (int): The depth to truncate the signature at.
        basepoint (bool, optional): Defaults to ``False``. If ``True``, then the path will have an additional point
            prepended to the start corresponding to the origin. If this is ``False`` then the signature transform is
            invariant to translations of the path. (Which may or may not be desirable.)
        stream (bool, optional): Defaults to ``False``. If ``False`` then the signature transform of the whole path is
            computed. If ``True`` then the signature of all intermediate paths are also computed.
    ...
    Returns:
        ...
        Given an input `torch.Tensor` of shape :math:`(N, C, L)`, and input arguments :attr:`depth`, :attr:`basepoint`,
        :attr:`stream`, :attr:`flatten`, then the return value is, in pseudocode:
        ...
        if flatten:
            if stream:
                if basepoint:
                    ...
                    return torch.Tensor of shape (N, C + C^2 + \cdots + C^d, L - 1)
            else:
                return torch.Tensor of shape (N, C + C^2 + \cdots + C^d)
        else:
            if stream:
                if basepoint:
                    out = []
