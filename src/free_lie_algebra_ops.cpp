...
namespace signatory {
    namespace fla_ops {
        namespace detail {
            ...
            void set_indices(std::vector<std::vector<LyndonWord>>& lyndon_words, const misc::LyndonSpec& lyndonspec) {
        ...
        void lyndon_word_generator(std::vector<std::vector<LyndonWord>>& lyndon_words,
            ...
            lyndon_words.reserve(lyndonspec.depth);
            ...
            while (word.size()) {
                ++word.back();
                lyndon_words[word.size() - 1].emplace_back(word, false, lyndonspec);
                int64_t pos = 0;
            ...
            detail::set_indices(lyndon_words, lyndonspec);
        }
        ...
        void lyndon_bracket_generator(std::vector<std::vector<LyndonWord>>& lyndon_words,
            ...
            lyndon_words[0].reserve(lyndonspec.input_channels);
            for (int64_t channel_index = 0; channel_index < lyndonspec.input_channels; ++channel_index) {
                lyndon_words[0].emplace_back(std::vector<int64_t> {channel_index}, true, lyndonspec);
            }
        ...
        void lyndon_words_to_lyndon_basis(std::vector<std::vector<LyndonWord>>& lyndon_words,
            ...
            anagram_class_sizes.reserve(num_lyndon_words(lyndon_words, lyndonspec));
        ...
        torch::Tensor compress(const std::vector<std::vector<LyndonWord>>& lyndon_words, torch::Tensor input,
            ...
            if (sigspec.stream) {
                compressed = torch::empty({sigspec.output_stream_size,
                                           num_lyndon_words(lyndon_words, sigspec),
                                           sigspec.batch_size},
                                          sigspec.opts);
            }
            else {
                compressed = torch::empty({num_lyndon_words(lyndon_words, sigspec),
                                           sigspec.batch_size},
                                          sigspec.opts);
            }
